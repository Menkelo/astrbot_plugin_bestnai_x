...
- **中文自动翻译**：普通 `/nai` 与 `/nai5` 开启后会把中文描述翻译为 Danbooru / NovelAI 英文 tag；原始提示词模式不会调用翻译或 Danbooru。画布节点勾选「原始提示词」后会多出一个「翻译」子开关，用于只放开中文翻译，画师预设与质量词依旧不追加；该子开关仅对画布节点有效，`/nai0` 与 `/nai50` 指令仍然完全不翻译。
...
- 连接 `图片 -> 提示词` 后会立即读取或反推原图 tags，并准备默认收起的原图标签卡，不必先生成一次；点击"生成"时再合并手写提示词并生成结果。原图未变化时复用反推缓存，中文提示词未变化时复用翻译缓存。手写提示词会自动加上正向权重，不会被反推出的几十个 tags 淹没。
...
- **一键复用参数**：大图预览的“复用参数”会新建提示词节点，带入原始完整提示词、模型、尺寸、种子、负面提示词、采样参数和角色信息。新节点使用原始提示词模式避免重复追加画师/质量词，点击“生成”才会调用接口；高级参数里可编辑种子与负面词。步数和引导仍遵守当前生成范围。
- **明确的参数优先级**：手动值 → 原图值 → 默认值；Rescale 的 `0` 是有效手动值。界面显示、滑条和请求使用同一套规则。
- **附加面板高度与滚动**：原图标签和角色模块使用固定的 620px 正文高度上限，平移、缩放、拖动节点及窗口变化不会改变上限。长内容可在面板内滚动，节点重绘保留滚动位置；角色 X/Y 坐标在编辑卡标题行居中，窄屏时换行居中。
- **原图标签控制**：反推统一把用户新增的同类标签视为替换（例如 `standing` 替换 `sitting`、新服装替换旧服装），同时保留不相关的视线、鞋子和背景。附着式「原图标签」卡按分类显示 `英文 tag / 中文名`，每个分类可设为自动、锁定原图或移除原图；**点击单条标签可只划掉它**，同类其余标签及权重保留。单条和整类移除在原始提示词模式中也会生效，连接原图时同步作用于角色正向提示词；手写主提示词仍可主动加入标签。正文使用统一的细滚动条，并在滚动到边界后把滚轮交还画布缩放。
- **常驻角色模块**：每个提示词节点上方都有独立角色模块，无需连接原图即可添加、删除、启用或禁用角色。纯色位置板上的编号圆点支持点击切换正面/负面编辑器和拖动定位；提供均匀横向、三人构图、四人构图与随机位置，并可恢复原图角色参数或清空手动角色。布局会按当前有效角色数自动选择：单角色按出场顺序，两个及以上角色按坐标分区。角色数据和展开状态均随工作区保存。
- **原图内嵌参数复用**：上传的图片若带有可信的生成参数，反推时会直接读出原始提示词并跳过视觉模型；种子仍在时，用「原始提示词 + 你手写的提示词 + 同一种子」重新生成，还原度远高于视觉反推。支持三种内嵌格式：NovelAI PNG（`Comment` JSON）、NovelAI JPEG/WebP 官方导出（EXIF `UserComment` JSON，NovelAI 新版默认下载格式即 WebP）、SD WebUI 的 `parameters` 三段式文本（PNG tEXt 或 JPEG/WebP EXIF）。原图的采样参数（步数 / 引导系数 / CFG 重缩放 / 噪声计划 / 采样器）也会一并沿用，高级参数卡顶部会写明本次沿用了哪些值。若只保留了可信提示词而种子已丢失，仍可跳过反推，但会使用随机种子。普通图片的 `Description` 与无关 EXIF 备注不会被误当成 tags。V4+ 的角色提示词（`char_captions`）会保留官方 `centers` 精确坐标，默认并回还原 tags、自动折叠重复的人数标签，并固定透传结构化角色参数；不支持该字段的网关会自动回退为普通提示词生成。
...
- **多格式图片与 GIF 反推**：QQ 和画布支持 PNG、JPEG/JFIF、WebP、GIF、BMP、TIFF、ICO、AVIF。反推前统一转为静态 PNG：动画默认取首帧，开头完全透明时最多检查前 12 帧；TIFF 取第一页，并处理旋转和颜色模式。原文件与内嵌生成信息保持完整，GIF 仍可动态预览，下载和 ZIP 导出保留原格式。图片最大 15 MB、3000 万像素；反推副本会缩至最长边 2048 像素、最大 8 MB。AVIF 需 Pillow 11.3.0 及以上且包含对应解码器，HEIC/HEIF 请先转换为 PNG/JPEG。
...
- 素材库按画师整理成唱片堆，可见区域按需读取缩略图，原图只在预览、放入画布和导出时获取，并使用有容量上限的缓存。翻页保留信息栏状态，快速连续点击以最后一次操作为准；需要使用素材时从预览页点击「放入画布」。
...
- **错误诊断**：分别提示鉴权、代理、访问拦截、源站故障和限流；节点生成失败后可点击“复制诊断信息”，复制上游 HTTP 状态与已脱敏的原始错误。超时不会自动重复提交生图。
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `prompt_block_enabled` | bool | `true` | QQ 防护总开关。检测到敏感词时保留原始提示词，原图**不直接发群**，匿名上传图床后在群里引用指令消息发送临时链接；关闭后不再检测，图片原样直发 |
| `prompt_block_words` | text | 精简内置检测词 | 使用大文本框编辑，敏感词之间以中文逗号 `，` 分隔；清空文本框则不检测任何词。默认只拦性器官、下体和露点，泳装 / 内衣 / 透视等轻度暴露不触发保护 |

> 命中 NSFW 后原图会匿名上传到 storage.to（临时图床，链接带有效期自动过期），上传走生图接口同一份 `proxy` 配置；直连不通时在生图接口配置里填代理即可。上传失败只提示重试，不会把原图直接发群。
...
| `/添加画师 <预设名> <提示词>` | 远程收录画师预设并写入配置（立即生效、重启保留）；预设名限中英文/数字/下划线 1-20 字 |
...
本插件为 QQ 场景提供了提示词检测和 NSFW 图片防封处理：

1. **提示词敏感词检测**（`prompt_block_enabled`，默认开）
   - `prompt_block_words` 是完整检测词列表，配置页使用大文本框，词与词之间以中文逗号 `，` 分隔；可直接增删，清空后不检测任何词。命中时只记录触发状态，不改写正向提示词。
   - 内置词表只覆盖**性器官、下体、露点、全裸、性行为和未成年 / 非自愿红线**。分级元标签（`nsfw`、`explicit`、`r18`、`hentai`、`色情`、`涩图` 等）和 `bikini`、`lingerie`、`see-through`、`泳装` 一类轻度暴露词**不在其中**，这些图片会原样直发、不触发保护。需要更严的口径，自行把这些词加回文本框即可。
   - 空格、下划线和连字符按同一种分隔方式检测，可过滤 `nude`、`nude_female`、`oral-sex` 等 Danbooru 变体；零宽字符和 `rating:` 命名空间也无法绕过。
   - 检测只看提示词，看不到成图。只写 `nsfw` 而不点名部位的提示词一样可能画出性器官，这类图不会触发保护——把 `nsfw`、`explicit` 加回词表可以换回旧版的兜底行为。
2. **命中后的原图转发图床链接**（同样受 `prompt_block_enabled` 控制）
   - 只要本轮提示词检测命中敏感词，原图**不会直接发到群里**（避免 QQ 图像风控）：机器人把原图匿名上传到 storage.to，然后**引用发起者的指令消息**在群里发送临时链接，任何群成员点开即看。
   - 链接自带有效期自动过期、原图不落地本地；上传走生图接口同一份 `proxy` 配置。上传失败时只提示重试，不会把原图直接发群。

> 第 1、2 条共用 `prompt_block_enabled` 一个开关：检测正向提示词，命中后原图改为图床链接发送。提示词和负面提示词均保持用户原文，关闭开关意味着插件完全不再检测，图片原样直发。
...
└── services/
    ...
    ├── storage_to.py        # storage.to 匿名图床上传（NSFW 原图链接通道）
...
画布入口是 `pages/canvas/canvas.js`；预览、角色编辑、素材库、图片缓存与参数规则分别位于 `image-viewer.js`、`character-editor.js`、`asset-library.js`、`asset-cache.js` 和 `generation-params.js`。直接使用浏览器 ES modules，不需要打包。发版时同步入口及模块 URL 的缓存版本号。

运行 `python -m pytest tests -q` 检查后端与页面约束。浏览器回归需要安装可选开发依赖 `playwright` 并执行 `playwright install chromium`，随后可运行 `python -m pytest tests/test_canvas_viewer_browser.py -q`。测试使用本地接口夹具，不会调用计费生图服务。

图片反推参考了 [GPT-Image2-Skill](https://github.com/wuyoscar/GPT-Image2-Skill) 的视觉锚点、空间层次与可见证据检查方法，并适配为 NAI 标签输出；归属见 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。
