...
- 素材库使用无标题、无筛选工具的纯图片面板，固定每行显示三张正方形缩略图，右上角保留画师标签；面板右缘与顶部状态栏对齐，并与状态栏、预览框保持一致间距。素材库支持开启删除模式后多选批量删除；大量素材滚动到临近区域时才继续分批渲染，同时图片仍在页面启动后后台预加载。点击图片可查看原图、提示词与英文 tags，拖到画布才创建图片节点且不会收起素材库；删除画布节点不会删除素材库收藏。放大预览仅保留提示词与英文 tags，长内容使用细滚动条；素材面板独立接管滚轮，并仅通过顶部“素材库”按钮展开或收起。
