...
- **原图内嵌参数复用**：上传的图片若带有可信的 NovelAI PNG 参数，反推时会直接读出原始提示词并跳过视觉模型；种子仍在时，用「原始提示词 + 你手写的提示词 + 同一种子」重新生成，还原度远高于视觉反推。若只保留了可信提示词而种子已丢失，仍可跳过反推，但会使用随机种子。普通 PNG 的 `Description` 不会被误当成 NAI tags。V4+ 的角色提示词（`char_captions`）也会一并并入还原 tags，在画布生成中生效。
