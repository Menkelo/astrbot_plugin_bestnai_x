...
- **画布参数缓存回退**：原图被重编码而丢失 PNG 元数据时，首次反推得到的种子与 tags 会写回画布节点；之后修改手写覆盖词、保存到素材库或重新放回画布，都只复用这份缓存，不会重复调用视觉反推。反推生图会继续沿用这枚种子，便于替换角色、服装、动作或画师串时保持构图方向；提示词、画幅、步数等设置改变后不承诺像素级复现。
