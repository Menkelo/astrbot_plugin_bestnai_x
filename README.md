...
## Studio 工作区（仿 NovelAI）

画布顶栏的 `Frame` 图标按钮可切换到 **BestNAI Studio**——一个仿 NovelAI 官网的暗色表单式工作区，两个工作区共存于同一页面（`#studio-shell` 覆盖层显隐切换，不做整页跳转，因此不受 AstrBot 页面 `asset_token` 60 秒有效期的限制），并共用同一份后端配置与素材库。

- **左侧参数面板**：提示词、负面提示词（UC，留空沿用插件配置）、画幅、画师预设、步数、引导 Scale、CFG Rescale、采样器、噪声调度、种子与 Raw 模式；面板参数会保存在浏览器本地，下次打开自动恢复。
- **右侧预览区**：大图预览 + 底部会话历史缩略图条；元数据条显示种子（点击可复制并回填）、尺寸、步数、Scale 与采样器，支持下载。
- **发送到无限画布**：把当前生成结果作为图片节点放入画布，继续用节点连线做反推与迭代。
- 中文提示词自动翻译并复用翻译缓存；`Ctrl/⌘ + Enter` 快捷生成。
