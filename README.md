...
- 连接 `图片 -> 提示词 -> 反推图片` 后点击"生成"，会识别原图 tags、合并新提示词并生成结果；原图和角色保持设置未变化时复用反推缓存，中文提示词未变化时复用翻译缓存。
...
- **步数与引导系数**：工具栏的滑块图标可调整，本画布全局生效并持久化；留空表示沿用插件配置的默认值。
- 图片卡片底部显示该图的**种子 ID**，可用于复现；点击图片可查看完整提示词与英文 tags。
- 提示词节点可展开英文 tags；素材库支持图片预加载、电脑端拖入画布和手机端预览后放入画布。手机端支持单指平移与双指缩放。
