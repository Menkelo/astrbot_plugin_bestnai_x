...
- 图片与提示词使用同一个素材列表并支持统一搜索；素材图片在页面启动后后台预加载，点击打开原图、原始提示词与英文 tags 详情，拖到画布才创建图片节点。删除画布中的图片节点不会删除素材库收藏，素材只能通过素材库自身的移除按钮删除。素材面板中的横图占满整行且不会被面板高度压缩，方图与竖图使用双列瀑布流，并独立接管滚轮；拖拽预览保持原卡片尺寸和鼠标抓取位置，空素材库使用无边框居中状态，面板仅通过顶部“素材库”按钮展开或收起。
