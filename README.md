...
- 图片与提示词使用同一个素材列表并支持统一搜索；素材图片在页面启动后后台预加载，点击打开原图、原始提示词与英文 tags 详情，拖到画布才创建图片节点。素材面板独立接管滚轮，拖拽预览保持原卡片尺寸和鼠标抓取位置；不同宽高比的素材卡片保持各自高度，空素材库使用无边框居中状态，面板仅通过顶部“素材库”按钮展开或收起。
