...
- **调试模式**：在画布顶栏点击 bug 按钮即可开关；运行后底部状态栏会折叠显示当前节点的阶段耗时、提示词流水、请求参数和上游报错原文，后端仅在该次画布请求开启时返回调试数据。详见[配置说明](#7-debug_mode---调试模式)。
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `debug_mode` | bool | `false` | 兼容旧配置；画布内可用顶栏 bug 按钮独立开关调试状态 |

在画布内开启后，每次生图 / 反推都会在底部调试栏留下这一轮的流水，同样的内容也会打进 AstrBot 后端日志：
