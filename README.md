...
- 提示词节点提供可展开的“英文 tags”区域，生成或反推后会保存翻译得到的英文 tags；中文提示词未变化时会复用对应翻译，调整画幅、画师或重新生成不会重复调用翻译模型。展开时保持折叠栏位置并向下增加卡片高度，收起时恢复展开前高度，展开状态和节点尺寸会一并保存，修改主提示词时自动清除过期结果。
...
- 图片与提示词使用同一个素材列表并支持统一搜索；素材图片在页面启动后后台预加载，点击打开原图、原始提示词与英文 tags 详情，拖到画布才创建图片节点。删除画布中的图片节点不会删除素材库收藏，素材只能通过素材库自身的移除按钮删除。素材面板中的横图占满整行，方图与竖图使用双列瀑布流，并独立接管滚轮；拖拽预览保持原卡片尺寸和鼠标抓取位置，空素材库使用无边框居中状态，面板仅通过顶部“素材库”按钮展开或收起。
