...
- **调试模式**：插件配置里打开 `debug_mode` 后，提示词卡片底部会多出一块「调试信息」面板，逐节点显示这一轮的各阶段耗时、提示词流水、请求参数和上游报错原文，后端日志同步输出同样内容。详见[配置说明](#7-debug_mode---调试模式)。
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `debug_mode` | bool | `false` | 排查问题时才开。开启后画布的提示词节点底部会多出一块「调试信息」面板 |

开启后，每次生图 / 反推都会在节点里留下这一轮的流水，同样的内容也会打进 AstrBot 后端日志：

- **各阶段耗时**：翻译、反推、生图各花了多久，以及总耗时。反推和翻译是并发跑的，两边分别计时
- **提示词流水**：输入原文 → 送去翻译的部分 → 译文 → 拼上画师预设和质量词后的最终提示词
- **请求参数**：模型、尺寸、迭代步数、引导系数、种子，以及反推用的提供商与模型
- **上游报错原文**：平时只显示「内容没通过服务商的审核」这类可操作的结论，调试模式下额外附上接口返回的原始报文

面板里的「复制全部」按钮会把整段流水拷成纯文本，反馈问题时直接贴出来即可。
...
| 提示词翻译 / 图片反推失败 | 上游接口拒绝，常见于内容审核、Key 失效、额度不足 | 报错已换成可操作的中文结论；要看接口返回的原文请打开 `debug_mode` |
