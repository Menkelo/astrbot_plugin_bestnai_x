...
- 素材库使用无标题、无筛选工具的纯图片面板，固定每行显示三张正方形缩略图，右上角保留画师标签且不再提供单张素材删除按钮。面板打开时左边缘会与顶部“素材库”按钮对齐；大量素材滚动到临近区域时才继续分批渲染，同时图片仍在页面启动后后台预加载。点击图片可查看原图、提示词与英文 tags，拖到画布才创建图片节点；删除画布节点不会删除素材库收藏。素材面板独立接管滚轮，拖拽预览保持原卡片尺寸和鼠标抓取位置，并仅通过顶部“素材库”按钮展开或收起。
