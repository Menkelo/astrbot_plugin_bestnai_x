...
- 图片节点右侧可连接提示词节点左侧；提示词节点只保留统一的“生成”按钮，连接原图时自动按 QQ 反推流程把用户的新提示词与原图 tags 合并，再从提示词节点右侧生成图片，形成 `图片 -> 提示词 -> 反推图片` 链路。
...
- 图片与提示词使用同一个素材列表并支持统一搜索；素材图片在页面启动后后台预加载，卡片标题优先显示原始提示词，点击打开原图、原始提示词与英文 tags 详情，拖到画布才创建图片节点。删除画布中的图片节点不会删除素材库收藏，素材只能通过素材库自身的移除按钮删除。素材面板独立接管滚轮，拖拽预览保持原卡片尺寸和鼠标抓取位置；不同宽高比的素材卡片保持各自高度，空素材库使用无边框居中状态，面板仅通过顶部“素材库”按钮展开或收起。
