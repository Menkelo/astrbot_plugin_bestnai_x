...
- **基础生图**：`/nai <提示词>` 一键生图。
- **原始提示词生图**：`/nai0 <tag>` 跳过中文翻译，不追加画师串与质量提示词，仅沿用负面提示词；V5 模型下中文原样直通。
- **中文自动翻译**：开启后，`/nai` 中的中文描述会被自动翻译为 Danbooru / NovelAI 英文 tag。V5 原生支持中日文提示，中文不再强制翻译、也不会被非 ASCII 清理剥离（4.5 及以下维持原行为）。
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `char_structured` | bool | `false` | 结构化角色参数透传。反推多角色原图时把 `char_captions` 作为 `characters` 参数发给网关做分区生成，仅当网关真正支持该参数时开启；默认关闭，走文本合并（角色描述并入提示词并自动折叠重复的人数标签），兼容所有网关 |
| `nai0_model` | string | `nai-diffusion-4-5-full` | `/nai0` 使用的模型（4.5 / V5），提供商跟随所选模型 |
| `canvas_model` | string | `nai-diffusion-4-5-full` | 画布新建提示词节点的默认模型，可在每个节点的「模型」下拉里单独切换 |
...
| `/nai0 <tag>` | 原始提示词模式。跳过翻译，不追加画师串和质量提示词，仍沿用负面提示词与安全审核；模型（4.5/V5）在插件面板选择 |
