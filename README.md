...
| `prompt_block_words` | text | 精简内置检测词 | 使用大文本框编辑，敏感词之间以中文逗号 `，` 分隔；清空文本框则不检测任何词。默认只拦性器官、下体和露点，泳装 / 内衣 / 透视等轻度暴露不触发混淆 |
| `nsfw_storage_link_enabled` | bool | `false` | 命中敏感词并对群内图片混淆时，把未混淆原图匿名上传 storage.to 并将临时链接私聊发给发起者（自己看原图用）；上传走生图接口同一份代理配置 |
...
| `/解混淆` | 引用（回复）NSFW 混淆图后发送，机器人还原原图并把 storage.to 链接私聊发给请求者，群内不出现链接 |
...
1. **提示词敏感词检测**（`prompt_block_enabled`，默认开）
   ...
   - 内置词表只覆盖**性器官、下体、露点、全裸、性行为和未成年 / 非自愿红线**。分级元标签（`nsfw`、`explicit`、`r18`、`hentai`、`色情`、`涩图` 等）和 `bikini`、`lingerie`、`see-through`、`泳装` 一类轻度暴露词**不在其中**，这些图片会原样发送、不做混淆。需要更严的口径，自行把这些词加回文本框即可。
   - 空格、下划线和连字符按同一种分隔方式检测，可过滤 `nude`、`nude_female`、`oral-sex` 等 Danbooru 变体；零宽字符和 `rating:` 命名空间也无法绕过。
   - 检测只看提示词，看不到成图。只写 `nsfw` 而不点名部位的提示词一样可能画出性器官，这类图不会被混淆——把 `nsfw`、`explicit` 加回词表可以换回旧版的兜底行为。
2. **命中后的图片混淆**（同样受 `prompt_block_enabled` 控制）
   ...
   - 命中后图片下方会提示「引用图片并回复“/解混淆”可获取原图链接」——混淆算法可逆，群成员引用该图并发送 `/解混淆`，机器人还原后把原图链接私聊发给请求者。
3. **NSFW 原图私发通道**（`nsfw_storage_link_enabled`，默认关）
   - 开启后，命中敏感词时除群内混淆副本外，未混淆原图会被匿名上传到 storage.to，临时链接**私聊**补发给发起者（方便自己查看原图）。原图不落地本地，链接带有效期自动过期；上传走生图接口同一份 `proxy` 配置。
   - `/解混淆` 指令不依赖此开关，任何群成员都能引用混淆图自行获取原图链接。
