...
| `prompt_block_words` | text | 精简内置检测词 | 一行一个词，文本框内容即完整生效列表，可直接增删；清空文本框则不检测任何词 |
...
1. **提示词敏感词检测**（`prompt_block_enabled`，默认开）
   - `prompt_block_words` 是完整检测词列表，配置页以多行文本框显示，一行一个词；可直接增删，清空后不检测任何词。命中时只记录触发状态，不改写正向提示词。
