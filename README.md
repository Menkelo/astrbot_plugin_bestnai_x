...
- **Infinite Canvas**：在 AstrBot WebUI 中提供可平移、缩放和持久化的生图画布，支持提示词、图片与备注节点。

---

## Infinite Canvas

AstrBot `4.14.0` 及以上版本会在插件详情页识别 `Canvas` Page。打开后即可使用无限画布，不需要单独启动 Web 服务。
...
- 鼠标滚轮缩放、拖动空白区域平移、节点拖拽、适配视图、撤销/重做与删除；支持 Ctrl 框选、Ctrl 点击多选、成组拖动、批量删除与自动整理选中节点。
- 右键或双击空白处可打开节点创建菜单；右下角小地图支持点击和拖动定位，点击连线中点的“×”或双击线身可删除连线。
