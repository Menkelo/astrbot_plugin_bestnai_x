...
AstrBot `4.14.0` 及以上版本会在插件详情页识别 `Canvas` Page，不需要单独启动 Web 服务。入口首先显示项目工作台；每个项目可创建多块独立画布，进入画布卡片后才加载对应的节点工作区。

项目工作台支持：

- 左侧项目列表与右侧可平移、缩放的画布卡片面板，布局、卡片和新建过渡复刻自 `hero8152/Infinite-Canvas`。
- 新建、重命名和删除项目；新建、重命名、移动、删除、恢复及彻底删除画布。
- 每块画布使用独立工作区文件，节点、连线和视口状态互不影响；旧版单一 `workspace.json` 会自动迁移为“默认项目”下的“迁移画布”。

独立画布支持：

- 双击空白处或点击工具栏创建提示词节点；节点可独立选择比例、画师预设与原始提示词模式。
- 图片节点右侧可连接提示词节点左侧，在提示词节点中执行“反推原图”；反推 tags 与原图比例会写回节点，再从右侧生成新的图片节点。
- 生成结果自动创建为图片节点，并与来源提示词建立连线；聊天命令与画布共用翻译、画师预设和并发限制。
- 画布页面整体复刻自 `hero8152/Infinite-Canvas`：浮动工具栏、全屏点阵画布、圆角节点、外置端口、贝塞尔连线与 DOM 小地图均采用其页面结构和表现方式。
- 鼠标滚轮缩放、拖动空白区域平移、Ctrl 框选、Ctrl 点击多选、成组拖动、批量删除、撤销/重做与自动整理选中节点。
- 右键或双击空白处可打开节点创建菜单；右下角小地图支持点击和拖动定位，点击连线中点的删除按钮或双击线身可删除连线。
- 拖放或选择本地图片创建图片节点，添加备注节点记录迭代方向。
- 图片与提示词素材库支持搜索、复用和删除；生成图、上传图可收藏为图片素材，选中的提示词节点可保存为提示词素材。
