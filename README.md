...
- **原图标签控制**：反推统一把用户新增的同类标签视为替换（例如 `standing` 替换 `sitting`、新服装替换旧服装），同时保留不相关的视线、鞋子和背景。附着式「原图标签」卡按分类显示 `英文 tag / 中文名`，每个分类可设为自动、锁定原图或移除原图；**点击单条标签可只划掉它**，同类的其余标签照常参与合并（反推出 `blue_eyes, closed_eyes` 时只去掉后者）。卡片内另有「复制全部」（只复制未划掉的标签）与「恢复划掉」。检测到 V4+ 多角色参数时，卡片还会显示「角色参数」面板，可单独启用/禁用角色、编辑角色正面/负面提示词、修改精确 X/Y 坐标、切换按坐标分区与出场顺序，并支持恢复原图角色参数。点击画布空白处会自动收起，节点未选中时滚轮仍用于缩放画布。
