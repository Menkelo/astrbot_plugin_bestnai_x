...
- **基础生图**：`/nai <提示词>` 一键生图。
- **原始提示词生图**：`/nai0 <tag>` 不追加画师串与质量提示词，仅沿用负面提示词；中文提示词自动翻译成英文 tags（所有模型同策略）。
- **中文自动翻译**：开启后，中文描述会被自动翻译为 Danbooru / NovelAI 英文 tag。所有模型同策略（含 V5 与 /nai0）——V5 对中文自然语言与角色别名的理解不稳定，统一翻译成英文 tags 更可靠。
...
| `/nai0 <tag>` | 原始提示词模式。不追加画师串和质量提示词，仍沿用负面提示词与安全审核；中文自动翻译成英文 tags（所有模型同策略） |
