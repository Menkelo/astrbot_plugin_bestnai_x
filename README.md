...
- 提示词节点提供可展开的“英文 tags”区域，生成或反推后会保存翻译得到的英文 tags；中文提示词未变化时会复用对应翻译，调整画幅、画师或重新生成不会重复调用翻译模型。展开时按实际内容自动增加卡片高度，不再保留多余底部空白；收起时恢复展开前高度，展开状态和节点尺寸会一并保存，修改主提示词时自动清除过期结果。
...
- 手机端使用两层状态栏完整保留图片、提示词、备注、素材库及画布管理工具，支持单指平移和以双指中心为锚点的捏合缩放；极窄屏幕可横向滑动工具行。移动端会停用小地图和高开销模糊效果，并按帧合并视口与连线更新；连线端点依据卡片实际渲染尺寸定位，避免窄屏卡片缩放后出现偏移和视觉断线。
- 选中卡片后，提示词、英文 tags 与备注编辑区才会接管鼠标滚轮；未选中卡片时滚轮继续缩放画布。
- 鼠标滚轮缩放、拖动空白区域平移、Ctrl 框选、Ctrl 点击多选、成组拖动、批量删除、撤销/重做与自动整理选中节点；编辑提示词或备注时，撤销/重做仅作用于当前文本。
- 点击任意节点会将其提升到最上层，重叠节点可按当前操作顺序切换前后层级。
- 双击空白处可创建提示词节点；按住鼠标中键可从节点或空白处平移画布，右键菜单可在当前位置添加图片、提示词或备注。右下角小地图支持点击和拖动定位，点击连线中点的删除按钮或双击线身可删除连线。
- 拖放或选择本地图片创建图片节点；图片节点支持右下角等比例对角缩放，备注节点可用于记录迭代方向。
- 素材库使用无标题、无筛选工具的纯图片面板，固定每行显示三张正方形缩略图，右上角保留画师标签；面板左缘与顶部“素材库”按钮对齐、右缘与状态栏对齐，并与状态栏、预览框保持一致间距。图片从面板顶部直接排列，底部常驻显示已收录图片数；“多选”进入选择模式后显示已选数量以及唯一的删除操作，按钮顺序为删除、取消，切换模式不会改变当前滚动位置；大量素材滚动到临近区域时才继续分批渲染，同时图片仍在页面启动后后台预加载。电脑端点击图片可查看原图、提示词与英文 tags，拖到画布才创建图片节点且不会收起素材库；手机端纵向滑动浏览素材，点按进入预览后使用“放入画布”按钮添加，避免拖拽与滚动冲突。删除画布节点不会删除素材库收藏。放大预览仅保留提示词与英文 tags，长内容使用细滚动条；素材面板独立接管滚轮，并仅通过顶部“素材库”按钮展开或收起。
- 图片放大详情点击内容外空白即可关闭；提示词与英文 tags 支持按钮复制和鼠标拖选复制。纯英文提示词不再重复展示两份相同文本，只保留英文 tags。
- 图片节点下载使用独立 Blob 地址，不会卸载当前画布页面；后台生图或反推期间下载已有图片不会中止正在执行的请求。
- 生图、反推、复制及其他画布操作提示统一显示在顶部状态栏中央，新提示会替换旧提示；提示层级高于图片放大遮罩，预览图片时也能清晰看到操作结果。
- 项目按钮使用与其他状态栏工具一致的圆形叠层样式；项目菜单与素材库互斥展开，并共用左右对齐、顶部间距和圆角规格。
