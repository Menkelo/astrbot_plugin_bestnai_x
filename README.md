...
- 双击空白处或点击工具栏创建提示词节点；节点可独立选择比例、画师预设与原始提示词模式，最后一次选择的比例和画师会由后续新节点沿用，悬停“原始提示词”可查看其用途。
- 提示词节点连接原图后才显示“角色保持”及可选角色名：填写时视觉模型优先输出该角色的标准角色与作品 tags；留空时模型先识别角色身份，无法确认时保留显著外观特征。
- 图片节点右侧可连接提示词节点左侧；提示词节点只保留统一的“生成”按钮，连接原图时自动按 QQ 反推流程把用户的新提示词与原图 tags 合并，但保持提示词节点当前选择的画幅比例，再从提示词节点右侧生成图片，形成 `图片 -> 提示词 -> 反推图片` 链路。
- 同一张原图的反推 tags 会随画布保存；链接图片、角色保持开关和角色名均未变化时，后续生成直接复用识图结果，不再调用反推模型。修改中文提示词只会让翻译缓存失效，不会让原图识别缓存失效。
- 提示词节点提供可展开的“英文 tags”区域，生成或反推后会保存翻译得到的英文 tags；中文提示词未变化时会复用对应翻译，调整画幅、画师或重新生成不会重复调用翻译模型。展开时使用不受画布缩放影响的节点尺寸固定折叠栏位置并向下增加卡片高度，收起时恢复展开前高度，展开状态和节点尺寸会一并保存，修改主提示词时自动清除过期结果。
- 提示词节点可在卡片内部选择画师预设，但标题栏保持简洁；生成图会保存后端实际采用的预设名，并在图片节点、放大详情和素材缩略图中显示。
- 生成结果自动创建为图片节点，并与来源提示词建立连线；首张结果会避开已有节点，同一提示词后续生成的图片按向右上方偏移的方式层叠，最新结果保持在最上层。聊天命令与画布共用翻译、画师预设和并发限制。
...
- 素材库使用无标题、无筛选工具的纯图片面板，固定每行显示三张正方形缩略图，右上角保留画师标签；面板左缘与顶部“素材库”按钮对齐、右缘与状态栏对齐，并与状态栏、预览框保持一致间距。开启“多选”后可批量选择并通过唯一的“删除所选”按钮删除素材，切换多选状态不会改变当前滚动位置；大量素材滚动到临近区域时才继续分批渲染，同时图片仍在页面启动后后台预加载。点击图片可查看原图、提示词与英文 tags，拖到画布才创建图片节点且不会收起素材库；删除画布节点不会删除素材库收藏。放大预览仅保留提示词与英文 tags，长内容使用细滚动条；素材面板独立接管滚轮，并仅通过顶部“素材库”按钮展开或收起。
- 图片放大详情点击内容外空白即可关闭；提示词与英文 tags 支持按钮复制和鼠标拖选复制。纯英文提示词不再重复展示两份相同文本，只保留英文 tags。
- 生图、反推、复制及其他画布操作提示统一显示在顶部状态栏中央，新提示会替换旧提示，避免在画布右上角堆叠占用空间。
