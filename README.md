...
- 单一顶部状态栏显示插件 Logo、插件名称、版本、作者、项目地址和服务连接状态；绿灯表示连接正常，红灯表示连接中断。

独立画布支持：

- 双击空白处或点击工具栏创建提示词节点；节点可独立选择比例、画师预设与原始提示词模式。
- 图片节点右侧可连接提示词节点左侧，在提示词节点中执行“反推”；画布会按 QQ 反推流程把用户的新提示词与原图 tags 合并，再从提示词节点右侧生成“反推图片”节点，形成 `图片 -> 提示词 -> 反推图片` 链路。
