...
- **反推冲突模式**：画布可选择「改图（覆盖冲突）」或「复刻（保留原图）」；QQ 可在提示词中使用 `--mode replicate`。角色、发型、服装、动作、构图和背景等标签会按分类去重与覆盖。
...
- **反推模式**：默认「改图」会把用户新增的同类标签视为替换（例如 `standing` 替换 `sitting`、新服装替换旧服装），但会保留不相关的视线、鞋子、背景等标签；「复刻」只在明确写出替换指令时清理原图分类，适合在原图基础上追加描述。QQ 示例：`/nai --mode replicate 加一点夕阳`。
- **原图内嵌参数复用**：上传的图片若带有可信的 NovelAI PNG 参数，反推时会直接读出原始提示词并跳过视觉模型；种子仍在时，用「原始提示词 + 你手写的提示词 + 同一种子」重新生成，还原度远高于视觉反推。若只保留了可信提示词而种子已丢失，仍可跳过反推，但会使用随机种子。普通 PNG 的 `Description` 不会被误当成 NAI tags。
...
- 素材库支持图片预加载、电脑端拖入画布和手机端预览后放入画布；电脑端点击「展开」后会铺满工具栏下方并避开底部调试栏。
...
- 常用操作：双击空白处新建提示词，右键打开添加菜单，`Ctrl/⌘ + Enter` 生成，`Ctrl/⌘ + Z` 撤销，`Ctrl/⌘ + Shift + Z` 重做，`Delete` 删除选中节点。
