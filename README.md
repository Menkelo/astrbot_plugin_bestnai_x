...
- **标签图层控制**：反推统一把用户新增的同类标签视为替换（例如 `standing` 替换 `sitting`、新服装替换旧服装），同时保留不相关的视线、鞋子和背景。画布可在附着式标签图层卡中单独把某一分类设为自动、锁定原图或移除原图，不再维护容易互相冲突的两套全局反推模式。
...
- 素材库支持图片预加载、电脑端拖入画布和手机端预览后放入画布；打开素材库时会临时隐藏右下角缩略图，电脑端点击「展开」后会铺满工具栏下方并避开底部调试栏。
- 顶栏地图按钮可长期显示或隐藏右下角缩略图，选择会保存在当前浏览器中。
