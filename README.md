...
- 素材库将图片与提示词分为独立分页；支持搜索名称、提示词、英文 tags 与画师，并可按画师、比例、生成/反推来源筛选，按最新、最早或名称排序。图片默认使用不显示名称的纯缩略图紧凑网格，左上角悬浮按钮用于移除素材，右上角保留画师标签；缩略图大小可调，也可切换为保留原始比例的双列瀑布流，这些偏好会保存在浏览器中。大量素材滚动到临近区域时才会继续分批渲染，同时图片仍在页面启动后后台预加载。点击图片可查看原图、提示词与英文 tags，拖到画布才创建图片节点；删除画布节点不会删除素材库收藏，素材只能通过素材库自身的移除按钮删除。素材面板独立接管滚轮，拖拽预览保持原卡片尺寸和鼠标抓取位置，面板仅通过顶部“素材库”按钮展开或收起。
