...
class CanvasPageBridgeTest(unittest.TestCase):
    ...
    def test_library_preloads_and_uses_click_preview_with_drag_placement(self) -> None:
        ...
        self.assertIn('class="asset-tabs"', html)
        self.assertIn('data-asset-tab="image"', html)
        self.assertIn('data-asset-tab="prompt"', html)
        self.assertIn('data-asset-layout="compact"', html)
        self.assertIn('data-asset-layout="masonry"', html)
        self.assertIn('id="assetArtistFilter"', html)
        self.assertIn('id="assetRatioFilter"', html)
        self.assertIn('id="assetSourceFilter"', html)
        self.assertIn('id="assetSort"', html)
        self.assertIn('id="assetThumbSize"', html)
        ...
        self.assertIn('group.className = "asset-columns"', editor)
        self.assertIn("columns[0].offsetHeight <= columns[1].offsetHeight", editor)
        self.assertIn("renderImageAssetCard(item, target)", editor)
        self.assertIn('renderImageAssetCard(item, els.assetGrid, true)', editor)
        self.assertIn('state.assetUi.type === "image" && state.assetUi.layout === "masonry"', editor)
        self.assertIn(".asset-image-card.wide", styles)
        self.assertIn(".asset-grid.masonry-layout > *", styles)
        self.assertIn("flex: 0 0 auto;", styles)
        self.assertIn(".asset-columns {", styles)
        ...
        self.assertIn("function saveAssetPreferences()", editor)
        self.assertIn("function assetSourceKind(item)", editor)
        self.assertIn("function assetTimestamp(item)", editor)
