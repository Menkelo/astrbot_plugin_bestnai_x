...
function normalizeCanvas(canvas){
    return {
        ...canvas,
        project: canvas.projectId || canvas.project || 'default',
        board_x: canvas.x ?? canvas.board_x ?? 0,
        board_y: canvas.y ?? canvas.board_y ?? 0,
        node_count: canvas.nodeCount ?? canvas.node_count ?? 0,
        created_at: canvas.createdAt || canvas.created_at || '',
        updated_at: canvas.updatedAt || canvas.updated_at || '',
        deleted_at: canvas.deletedAt || canvas.deleted_at || '',
    };
}
...
function localResponse(data, ok=true, status=200){
    return {
        ...
        blob: async () => new Blob([JSON.stringify(data, null, 2)], { type:'application/json' }),
    };
}
...
async function canvasFetch(url, options={}){
    try {
        ...
        throw new Error(`不支持的项目工作台请求：${parsed.pathname}`);
    } catch(error){
        console.error(error);
        return localResponse({ error:error.message || '请求失败' }, false, 500);
    }
}
...
async function loadAll(){
    try {
        const [pRes, cRes] = await Promise.all([
            canvasFetch('/api/projects'),
            canvasFetch('/api/canvases')
        ]);
...
async function createProject(){
    ...
    closeNewProject();
    try {
        const res = await canvasFetch('/api/projects', {
            method: 'POST',
...
async function renameProject(pid, name){
    ...
    try {
        const res = await canvasFetch(`/api/projects/${encodeURIComponent(pid)}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name })
        });
...
async function deleteProject(pid){
    ...
    try {
        const res = await canvasFetch(`/api/projects/${encodeURIComponent(pid)}`, { method: 'DELETE' });
...
function openCreateCard(worldPt){
    ...
    el.innerHTML = `
        ...
        <div class="ws-create-toggle">
            <button class="ws-create-toggle-btn active" type="button" data-kind="classic">${L('NAI 画布','NAI canvas')}</button>
        </div>
...
async function createCanvasOnBoard(title, kind, worldPt){
    ...
    closeCreateCard();
    try {
        const res = await canvasFetch('/api/canvases', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                title: name,
...
function collectCanvasAssetIds(value, out = [], seen = new Set()){
    if(value == null) return out;
    if(Array.isArray(value)){
        value.forEach(item => collectCanvasAssetIds(item, out, seen));
        return out;
    }
    if(typeof value === 'object'){
        const assetId = String(value.assetId || '');
        if(/^[a-f0-9]{32}$/.test(assetId) && !seen.has(assetId)){
            seen.add(assetId);
            out.push(assetId);
        }
        Object.values(value).forEach(item => collectCanvasAssetIds(item, out, seen));
    }
    return out;
}
...
function decodeDataUrl(dataUrl){
    ...
    if(!match) throw new Error('invalid asset payload');
...
function assetExportName(assetId, mimeType, used){
    const extensions = {
        'image/png':'png',
        'image/jpeg':'jpg',
        'image/webp':'webp',
    ...
    let suffix = 2;
    while(used.has(name)){
        name = `resources/${assetId}-${suffix}.${extension}`;
        suffix++;
    }
    used.add(name);
    return name;
...
async function exportCanvasWithResources(id){
    ...
    try {
        ...
        const assetIds = collectCanvasAssetIds(cv).slice(0, 1000);
        ...
        for(const assetId of assetIds){
            try {
                ...
                manifest.push({ assetId, file:name, size:decoded.bytes.length });
            } catch(e) {
                skipped++;
                manifest.push({ assetId, skipped:true, reason:String(e?.message || e || 'asset failed').slice(0, 120) });
            }
        }
...
async function persistMeta(id, patch){
    try {
        const res = await canvasFetch(`/api/canvases/${encodeURIComponent(id)}/meta`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function deleteCanvas(id){
    ...
    if(!c) return;
    try {
        const res = await canvasFetch(`/api/canvases/${encodeURIComponent(id)}`, { method: 'DELETE' });
...
async function refreshTrashCount(){
    try {
        const res = await canvasFetch('/api/canvases/trash');
        if(!res.ok) return;
        const data = await res.json();
...
async function restoreCanvas(id){
    try {
        const res = await canvasFetch(`/api/canvases/${encodeURIComponent(id)}/restore`, { method: 'POST' });
...
async function purgeCanvas(id){
    try {
        const res = await canvasFetch(`/api/canvases/${encodeURIComponent(id)}/purge`, { method: 'DELETE' });
